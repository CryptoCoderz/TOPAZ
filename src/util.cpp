...
#else
    const char* pszModule = "TopazCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\TopazCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\TopazCoin
    // Mac: ~/Library/Application Support/TopazCoin
    // Unix: ~/.TopazCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TopazCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "TopazCoin.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
    {
        boost::filesystem::path ConfPath;
               ConfPath = GetDataDir() / "TopazCoin.conf";
               ...
               fprintf(ConfFile, "port=6909\n");
               fprintf(ConfFile, "rpcport=6908\n");
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "TopazCoind.pid"));
